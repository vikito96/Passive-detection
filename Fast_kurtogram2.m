function [c t tc f S nfft BBWW FFCC] = Fast_kurtogram2(x,nlevel,Fs)
...
%pcolor(freq_w,1:2*nlevel,Kwav),colormap(flipud(gray)),colorbar;
[I,J,M] = max_IJ(Kwav);
...
BBWW=Fs*2^-(Level_w(I)+1);FFCC=Fs*fi;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SIGNAL FILTERING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c = [];
...
% title(['Envelope of filtered signal, Bw=Fs/2^{',num2str(level+1),'}, fc=',num2str(Fs*fc),'Hz, Kurt=',num2str(fix(10*kx)/10)])
...
%    subplot(313),plot(f,S(1:nfft/2),'k'),title('Amplitude spectrum of the squared envelope'),xlabel('frequency [Hz]'),xlim([f(1) f(end)])
   ...
   %figure
   %plot(f,S(1:nfft/2),'k'),title('Amplitude spectrum of the squared envelope'),xlabel('frequency [Hz]'),xlim([f(1) f(end)])
end
end
